...
A light ground (`--color-bg` #f5f6f7) with `--color-text` #1d2d3e and two accents — `--color-accent` #0070f2 and `--color-accent-2` #b8541a. Dark mode is a second palette on the same roles — a violet accent (#8b6cf0) on a purple-tinted ink ground (#16121f), with the same orange secondary: it follows the OS unless you set `data-theme="light"`/`data-theme="dark"` on `<html>`, and `.theme-dark` flips a single subtree. On dark the ramps reverse (100 stays the subtlest fill, 900 the strongest value), so component classes need no changes. Each role carries a 100–900 tonal ramp (`--color-neutral-100` … `--color-accent-2-900`) generated in OKLCH on a shared perceptual lightness scale, so the same step of any ramp has the same visual weight. Use the light steps (100–300) for tinted fills, hovers and subtle borders, 500 as the role's base, and the dark steps (700–900) for text on tinted fills and for pressed states; prefer ramp steps over ad-hoc `color-mix()`. For elevation use `--shadow-sm/md/lg` (already tuned to the ground) rather than ad-hoc box-shadows.

### Data color

Status colors carry meaning — `--color-positive`, `--color-negative`, `--color-warning`, `--color-info` — and stay out of the categorical rotation. Charts use `--series-1` … `--series-8`, assigned in order and never repeated inside one chart; both palettes are tuned per mode, so a chart re-colors itself in dark. `--grid-line` is the axis/gridline hairline. Dense tables: `.table-dense`, `.table-zebra`, `.table-sticky`, with `.num` on numeric cells for tabular figures and right alignment; `.metric` / `.metric-value` / `.metric-label` for KPI blocks and `.delta-up` / `.delta-down` for change figures. `.chart`, `.chart-col`, `.chart-stack`, `.pie`, `.pie-donut` and `.legend` are CSS-only chart primitives — see the Data & charts card.
